...
class Kubus : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        rumus.setOnClickListener {
            val intent = Intent(this, RumusKubus::class.java)
        ...
        buttonHitung.setOnClickListener {
            ...
            if (sisi.isEmpty()) {
                Toast.makeText(this, "Masukkan nilai sisi kubus!", Toast.LENGTH_SHORT).show()
            } else if (!sisi.matches(Regex("^[0-9]*\\.?[0-9]+\$"))) {
                Toast.makeText(this, "Masukkan angka yang valid!", Toast.LENGTH_SHORT).show()
            } else {
                val s = sisi.toDouble()
                val volume = s * s * s // Rumus volume kubus: s³
                hasilVolume.text = "Volume: %.2f cm³".format(volume)
            }
        }

        inputSisi.setOnKeyListener { v, keyCode, event ->
