...
class Kerucut : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        rumus.setOnClickListener {
            val intent = Intent(this, RumusKerucut::class.java)
        ...
        buttonHitung.setOnClickListener {
            ...
            if (jariJari.isEmpty() || tinggi.isEmpty()) {
                ...
                val volume = (1.0 / 3.0) * Math.PI * r * r * t // Rumus volume kerucut: (1/3) × π × r² × t
                hasilVolume.text = "Volume: %.2f cm³".format(volume)
            }
        }

        inputJariJari.setOnKeyListener { _, keyCode, event ->
        ...
        inputTinggi.setOnKeyListener { _, keyCode, event ->
            if (event.action == KeyEvent.ACTION_DOWN && keyCode == KeyEvent.KEYCODE_ENTER) {
                buttonHitung.performClick()
                return@setOnKeyListener true
            }
